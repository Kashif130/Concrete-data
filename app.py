        ...
        "sections": [
            ...
            },
            {
                ...
                "body": (
                    ...
                    "- **Accepted article: +100 points** per week\n"
...
with st.sidebar:
    ...
    nav_labels = {k: f"{v['icon']}  {v['title']}" for k, v in KB.items()}
    selected = st.radio(
        "Navigation",
    ...
    st.markdown("<div style='font-size:9px; color:#333; letter-spacing:2px; margin-bottom:7px;'>QUICK ACCESS</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div style='font-size:9px; color:#333; letter-spacing:2px; margin-bottom:6px;'>SEARCH</div>", unsafe_allow_html=True)
    search_q = st.text_input("search", placeholder="topic search karo...", label_visibility="collapsed")
...
<div style='background:rgba(0,0,0,0.55); border:1px solid rgba(0,255,136,0.13);
  ...
  <div>
    <div style='font-size:17px; font-weight:bold; background:linear-gradient(90deg,#00ff88,#00ccff);
         -webkit-background-clip:text; -webkit-text-fill-color:transparent; letter-spacing:2px;'>
      CONCRETE NEXUS — COMPLETE GUIDE
    </div>
    <div style='font-size:9px; color:#444; letter-spacing:2px; margin-top:2px;'>
      Data sourced from: concrete.xyz • docs.concrete.xyz • points.concrete.xyz • enterprise.concrete.xyz
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
if search_q and len(search_q.strip()) > 1:
    ...
    for key, sec in KB.items():
        ...
        if hits:
            ...
            st.markdown("")
    if not found:
        st.markdown(f"<span style='color:#555;'>'{search_q}' ke liye results nahi mile. Sidebar se section browse karo.</span>", unsafe_allow_html=True)

    # ── Search ke baad relevant links ──
    ...
    if q_links:
        ...
        st.markdown(
            f"<div class='link-box'><div class='link-box-title'>🔗 '{search_q}' SE RELATED LINKS</div>{pills}</div>",
            unsafe_allow_html=True,
        )
    else:
        # Default helpful links
        render_links("overview")
...
# ── Normal Mode ────────────────────────────────────────────────────────────────
    ...
    for sub in page["sections"]:
        st.markdown(f"<div class='section-card'><div class='section-heading'>▸ {sub['heading']}</div></div>", unsafe_allow_html=True)
        st.markdown(sub["body"])
        st.markdown("")

    # ── Is section ke liye relevant links ──
...
<div class='footer-bar'>
  ...
  ⚠️ Educational purposes only — Not financial advice. DeFi involves significant risk. Always DYOR.
</div>
""", unsafe_allow_html=True)
